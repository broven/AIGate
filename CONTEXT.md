...
**Cost Multiplier**:
挂在 Provider 上的价格**校正**系数，用于把上游公布的价目表修正为真实支付金额（中转站自带倍率、或倍率为 0 的免费渠道）。它不是加价系数，也不是路由偏好旋钮——校正后的价格必须始终等于真实支出。倍率恰为 **0** 是在声明整个 Provider 免费：连没有标价的 Deployment 也解析为 0，而不是「价格未知」。
